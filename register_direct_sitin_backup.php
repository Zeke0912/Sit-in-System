...
if (isset($_POST['studentId']) && !empty($_POST['studentId']) &&
    ...
    if ($pc_number !== null) {
        // Check if the PC is under maintenance (highest priority check)
        $maintenance_check_sql = "SELECT id FROM pc_status WHERE lab_id = ? AND pc_number = ? AND status = 'maintenance'";
        $maintenance_check_stmt = $conn->prepare($maintenance_check_sql);
        $maintenance_check_stmt->bind_param("ii", $subjectId, $pc_number);
        $maintenance_check_stmt->execute();
        $maintenance_check_result = $maintenance_check_stmt->get_result();
        
        if ($maintenance_check_result->num_rows > 0) {
            echo '<div class="error-message" style="background-color: #f8d7da; border-left: 5px solid #9b59b6; padding: 15px; margin: 20px 0; border-radius: 4px;">
                ...
                <p>PC #' . $pc_number . ' is currently under maintenance and <strong>cannot be used</strong>. Please select a different PC.</p>
                <p>Maintenance PCs are displayed in purple with a diagonal striped pattern.</p>
            </div>';
            $maintenance_check_stmt->close();
            $conn->close();
            exit();
        }
        $maintenance_check_stmt->close();
        
        // Continue with other checks only if not under maintenance
