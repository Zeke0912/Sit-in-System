...
<script>
    ...
    // Add this code to force the PC grid layout after the page loads
    document.addEventListener('DOMContentLoaded', function() {
        // Force the PC grid to display more items per row
        setInterval(function() {
            const pcGrid = document.querySelector('.pc-grid');
            if (pcGrid) {
                pcGrid.style.display = 'grid';
                pcGrid.style.gridTemplateColumns = 'repeat(auto-fill, minmax(150px, 1fr))';
                pcGrid.style.gap = '10px';
                pcGrid.style.width = '100%';
            }
