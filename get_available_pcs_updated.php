...
while ($row = $result->fetch_assoc()) {
    $reserved_pcs[] = (int)$row['pc_number']; // Cast to integer to ensure consistency
}
...
$maintenance_pcs = [];
while ($row = $result->fetch_assoc()) {
    $maintenance_pcs[] = (int)$row['pc_number']; // Cast to integer to ensure consistency
    // Log maintenance PCs for debugging
    error_log("PC " . $row['pc_number'] . " in lab " . $subject_id . " is marked as maintenance");
}
...
$occupied_pcs = [];
while ($row = $result->fetch_assoc()) {
    $occupied_pcs[] = (int)$row['pc_number']; // Cast to integer to ensure consistency
}

// For debugging
error_log("Maintenance PCs in lab " . $subject_id . ": " . implode(", ", $maintenance_pcs));
...
echo json_encode([
    ...
    'maintenance_count' => count($maintenance_pcs)
]);
